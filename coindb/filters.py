...
class CoinFilterSet(django_filters.FilterSet):
    ...
    material = django_filters.ModelChoiceFilter(queryset=Material.objects.all(), field_name='material', label='Материал')
    ...
    def filter_term(self, queryset, name, value):
        # Если пользователь ввел несколько слов для поиска, будем искать по нескольким полям
        criteria = Q()
